...
template<typename Dtype>
__global__ FGPU_DEFINE_KERNEL(BRForward, const int count, const int inner_dim, const Dtype* in,
                          const Dtype* permut, Dtype* out) {
  fgpu_dev_ctx_t *ctx;
  ...
  FGPU_FOR_EACH_DEVICE_BLOCK(_blockIdx) {
    ...
    CUDA_KERNEL_LOOP(index, count, _blockIdx, _gridDim) {
      int n = index / (inner_dim);
      int in_n = static_cast<int>(FGPU_COLOR_LOAD(ctx, &permut[n]));
      FGPU_COLOR_STORE(ctx, &out[index],
              FGPU_COLOR_LOAD(ctx, &in[in_n * (inner_dim) + index % (inner_dim)]));
    }
...
template<typename Dtype>
__global__ FGPU_DEFINE_KERNEL(BRBackward, const int count, const int inner_dim,
                           const Dtype* in, const Dtype* top_indexes,
                           const Dtype* begins, const Dtype* counts,
                           Dtype* out) {
  fgpu_dev_ctx_t *ctx;
  ...
  FGPU_FOR_EACH_DEVICE_BLOCK(_blockIdx) {
    ...
    CUDA_KERNEL_LOOP(index, count, _blockIdx, _gridDim) {
      Dtype *out_addr = FGPU_COLOR_TRANSLATE_ADDR(ctx, &out[index]);
      int n = index / (inner_dim);
      *out_addr = 0;
      int lower = static_cast<int>(FGPU_COLOR_LOAD(ctx, &begins[n]));
      int upper = lower + static_cast<int>(FGPU_COLOR_LOAD(ctx, &counts[n]));
      for (int i = lower; i < upper; ++i) {
        int in_n = static_cast<int>(FGPU_COLOR_LOAD(ctx, &top_indexes[i]));
        *out_addr += FGPU_COLOR_LOAD(ctx, &in[in_n * (inner_dim) + index % (inner_dim)]);
      }
    }
...
#else
  FGPU_CHECK(FGPU_LAUNCH_KERNEL(BRForward<Dtype>,
      CAFFE_GET_BLOCKS(threads), CAFFE_CUDA_NUM_THREADS, 0,
      top[0]->count(), bottom[0]->count() / bottom[0]->shape(0),
      bottom[0]->gpu_data(), bottom[1]->gpu_data(), top[0]->mutable_gpu_data()));
...
void BatchReindexLayer<Dtype>::Backward_gpu(
  ...
  CUDA_POST_KERNEL_CHECK;
#else
   FGPU_CHECK(FGPU_LAUNCH_KERNEL(BRBackward<Dtype>,
      CAFFE_GET_BLOCKS(threads), CAFFE_CUDA_NUM_THREADS, 0,
      bottom[0]->count(), bottom[0]->count() / bottom[0]->shape(0),
      top[0]->gpu_diff(), top_indexes.gpu_data(), begins.gpu_data(),
      counts.gpu_data(), bottom[0]->mutable_gpu_diff()));

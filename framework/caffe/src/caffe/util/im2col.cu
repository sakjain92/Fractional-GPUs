    ...
    CUDA_KERNEL_LOOP(index, n, _blockIdx, _gridDim) {
      ...
      for (int i = 0; i < kernel_h; ++i) {
        for (int j = 0; j < kernel_w; ++j) {
          int h_im = h_offset + i * dilation_h;
          int w_im = w_offset + j * dilation_w;
          FGPU_COLOR_STORE(ctx, data_col_ptr,
              (h_im >= 0 && w_im >= 0 && h_im < height && w_im < width) ?
              FGPU_COLOR_LOAD(ctx, &data_im_ptr[i * dilation_h * width + j * dilation_w]) : 0);
          data_col_ptr += height_col * width_col;
        }
      }
    }
...
__global__ FGPU_DEFINE_KERNEL(im2col_nd_gpu_kernel, const int n, const Dtype* data_im,
  ...
  if (threadIdx.x < num_axes) {
    shared_dilation[threadIdx.x] = FGPU_COLOR_LOAD(ctx, &dilation[threadIdx.x]);
    shared_kernel_shape[threadIdx.x] = FGPU_COLOR_LOAD(ctx, &kernel_shape[threadIdx.x]);
    shared_pad[threadIdx.x] = FGPU_COLOR_LOAD(ctx, &pad[threadIdx.x]);
    shared_stride[threadIdx.x] = FGPU_COLOR_LOAD(ctx, &stride[threadIdx.x]);
  }
  if (threadIdx.x < num_axes + 1) {
    shared_col_shape[threadIdx.x] = FGPU_COLOR_LOAD(ctx, &col_shape[threadIdx.x]);
    shared_im_shape[threadIdx.x] = FGPU_COLOR_LOAD(ctx, &im_shape[threadIdx.x]);
  }
  __syncthreads();
    ...
    CUDA_KERNEL_LOOP(index, n, _blockIdx, _gridDim) {
      ...
      do {
        ...
        if (in_range) {
          ...
          FGPU_COLOR_STORE(ctx, data_col_ptr, FGPU_COLOR_LOAD(ctx, &data_im_ptr[data_im_offset]));
        } else {
          FGPU_COLOR_STORE(ctx, data_col_ptr, 0);
        }
        data_col_ptr += data_col_inc;
        incremented = false;
        for (i = num_axes - 1; i >= 0; --i) {
...
template <typename Dtype>
__global__ FGPU_DEFINE_KERNEL(col2im_gpu_kernel, const int n, const Dtype* data_col,
    const int height, const int width, const int channels,
    const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
      ...
      for (int h_col = h_col_start; h_col < h_col_end; h_col += 1) {
        for (int w_col = w_col_start; w_col < w_col_end; w_col += 1) {
          ...
          if (h_k % dilation_h == 0 && w_k % dilation_w == 0) {
            ...
            int data_col_index = (((c_im * kernel_h + h_k) * kernel_w + w_k) *
                                  height_col + h_col) * width_col + w_col;
            val += FGPU_COLOR_LOAD(ctx, &data_col[data_col_index]);
          }
        }
      }
      FGPU_COLOR_STORE(ctx, &data_im[index], val);
    }
...
template <typename Dtype, int num_axes>
__global__ FGPU_DEFINE_KERNEL(col2im_nd_gpu_kernel, const int n, const Dtype* data_col,
    const int* im_shape, const int* col_shape,
    const int* kernel_shape, const int* pad, const int* stride,
    const int* dilation, Dtype* data_im) {
  int d_im[num_axes];  // NOLINT(runtime/arrays)
    ...
    CUDA_KERNEL_LOOP(index, n, _blockIdx, _gridDim) {
      ...
      for (int i = 0; i < num_axes; ++i) {
        ...
        if (d_col_start[i] >= d_col_end[i]) {
          // Skip computation if the dimension is 0 at any spatial axis --
          // final val will be 0.
          FGPU_COLOR_STORE(ctx, &data_im[index], 0);
          done = true;
          break;  // for (int i = 0; i < num_axes; ++i)
        }
      }
      ...
      do {
        ...
        if (!skip) {
          ...
          val += FGPU_COLOR_LOAD(ctx, &data_col[final_offset]);
        }
        skip = false;
        incremented = false;
        for (int i = num_axes - 1; i >= 0; --i) {
...
#else
    FGPU_CHECK(FGPU_LAUNCH_KERNEL(im2col_gpu_kernel<Dtype>, 
      CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
      num_kernels, data_im, height, width, kernel_h, kernel_w, pad_h,
      pad_w, stride_h, stride_w, dilation_h, dilation_w, height_col,
      width_col, data_col));
...
void im2col_nd_gpu(const Dtype* data_im, const int num_spatial_axes,
  ...
  switch (num_spatial_axes) {
  case 1:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((im2col_nd_gpu_kernel<Dtype, 1>),  // NOLINT_NEXT_LINE(whitespace/operators)
        CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
        num_kernels, data_im, im_shape, col_shape,
        kernel_shape, pad, stride, dilation, data_col));
    break;
  case 2:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((im2col_nd_gpu_kernel<Dtype, 2>),  // NOLINT_NEXT_LINE(whitespace/operators)
        CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
        num_kernels, data_im, im_shape, col_shape,
        kernel_shape, pad, stride, dilation, data_col));
    break;
  case 3:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((im2col_nd_gpu_kernel<Dtype, 3>),  // NOLINT_NEXT_LINE(whitespace/operators)
        CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
        num_kernels, data_im, im_shape, col_shape,
        kernel_shape, pad, stride, dilation, data_col));
    break;
  case 4:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((im2col_nd_gpu_kernel<Dtype, 4>),  // NOLINT_NEXT_LINE(whitespace/operators)
        CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
        num_kernels, data_im, im_shape, col_shape,
        kernel_shape, pad, stride, dilation, data_col));
    break;
  case 5:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((im2col_nd_gpu_kernel<Dtype, 5>),  // NOLINT_NEXT_LINE(whitespace/operators)
        CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
        num_kernels, data_im, im_shape, col_shape,
        kernel_shape, pad, stride, dilation, data_col));
    break;
  case 6:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((im2col_nd_gpu_kernel<Dtype, 6>),  // NOLINT_NEXT_LINE(whitespace/operators)
        CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
        num_kernels, data_im, im_shape, col_shape,
        kernel_shape, pad, stride, dilation, data_col));
    break;
  case 7:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((im2col_nd_gpu_kernel<Dtype, 7>),  // NOLINT_NEXT_LINE(whitespace/operators)
        CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
        num_kernels, data_im, im_shape, col_shape,
        kernel_shape, pad, stride, dilation, data_col));
    break;
  case 8:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((im2col_nd_gpu_kernel<Dtype, 8>),  // NOLINT_NEXT_LINE(whitespace/operators)
        CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
        num_kernels, data_im, im_shape, col_shape,
        kernel_shape, pad, stride, dilation, data_col));
    break;
  case 9:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((im2col_nd_gpu_kernel<Dtype, 9>),  // NOLINT_NEXT_LINE(whitespace/operators)
        CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
        num_kernels, data_im, im_shape, col_shape,
        kernel_shape, pad, stride, dilation, data_col));
    break;
  case 10:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((im2col_nd_gpu_kernel<Dtype, 10>),  // NOLINT_NEXT_LINE(whitespace/operators)
        CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
        num_kernels, data_im, im_shape, col_shape,
        kernel_shape, pad, stride, dilation, data_col));
    break;
...
void col2im_gpu(const Dtype* data_col, const int channels,
  ...
  CUDA_POST_KERNEL_CHECK;
#else
    FGPU_CHECK(FGPU_LAUNCH_KERNEL(col2im_gpu_kernel<Dtype>, 
      CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS, 0,
      num_kernels, data_col, height, width, channels, kernel_h, kernel_w,
      pad_h, pad_w, stride_h, stride_w, dilation_h, dilation_w,
      height_col, width_col, data_im));
...
void col2im_nd_gpu(const Dtype* data_col, const int num_spatial_axes,
  ...
  CUDA_POST_KERNEL_CHECK;
    ...
    switch (num_spatial_axes) {
  case 1:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((col2im_nd_gpu_kernel<Dtype, 1>),  // NOLINT_NEXT_LINE(whitespace/operators)
          CAFFE_GET_BLOCKS(im_size), CAFFE_CUDA_NUM_THREADS, 0,
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im));
    break;
  case 2:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((col2im_nd_gpu_kernel<Dtype, 2>),  // NOLINT_NEXT_LINE(whitespace/operators)
          CAFFE_GET_BLOCKS(im_size), CAFFE_CUDA_NUM_THREADS, 0,
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im));
    break;
  case 3:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((col2im_nd_gpu_kernel<Dtype, 3>),  // NOLINT_NEXT_LINE(whitespace/operators)
          CAFFE_GET_BLOCKS(im_size), CAFFE_CUDA_NUM_THREADS, 0,
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im));
    break;
  case 4:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((col2im_nd_gpu_kernel<Dtype, 4>),  // NOLINT_NEXT_LINE(whitespace/operators)
          CAFFE_GET_BLOCKS(im_size), CAFFE_CUDA_NUM_THREADS, 0,
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im));
    break;
  case 5:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((col2im_nd_gpu_kernel<Dtype, 5>),  // NOLINT_NEXT_LINE(whitespace/operators)
          CAFFE_GET_BLOCKS(im_size), CAFFE_CUDA_NUM_THREADS, 0,
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im));
    break;
  case 6:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((col2im_nd_gpu_kernel<Dtype, 6>),  // NOLINT_NEXT_LINE(whitespace/operators)
          CAFFE_GET_BLOCKS(im_size), CAFFE_CUDA_NUM_THREADS, 0,
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im));
    break;
  case 7:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((col2im_nd_gpu_kernel<Dtype, 7>),  // NOLINT_NEXT_LINE(whitespace/operators)
          CAFFE_GET_BLOCKS(im_size), CAFFE_CUDA_NUM_THREADS, 0,
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im));
    break;
  case 8:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((col2im_nd_gpu_kernel<Dtype, 8>),  // NOLINT_NEXT_LINE(whitespace/operators)
          CAFFE_GET_BLOCKS(im_size), CAFFE_CUDA_NUM_THREADS, 0,
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im));
    break;
  case 9:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((col2im_nd_gpu_kernel<Dtype, 9>),  // NOLINT_NEXT_LINE(whitespace/operators)
          CAFFE_GET_BLOCKS(im_size), CAFFE_CUDA_NUM_THREADS, 0,
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im));
    break;
  case 10:
    FGPU_CHECK(FGPU_LAUNCH_KERNEL((col2im_nd_gpu_kernel<Dtype, 10>),  // NOLINT_NEXT_LINE(whitespace/operators)
          CAFFE_GET_BLOCKS(im_size), CAFFE_CUDA_NUM_THREADS, 0,
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im));
    break;

...
#define RD_WG_SIZE_1 128
#define RD_WG_SIZE_2 192
...
{
	T* t;
    int ret = fgpu_memory_allocate((void **)&t, sizeof(T)*N);
    if (ret < 0)
        exit(EXIT_FAILURE);
	return t;
}
...
{
    fgpu_memory_free((void*)array);
}

void device_sync(void)
{
    int ret = fgpu_color_stream_synchronize();
    if (ret < 0)
        exit(EXIT_FAILURE);
}
...
{
    int ret = fgpu_memory_copy_async((void*)dst, (void*)src, N*sizeof(T), FGPU_COPY_GPU_TO_GPU);
    if (ret < 0)
        exit(EXIT_FAILURE);
...
{
    int ret = fgpu_memory_copy_async((void*)dst, (void*)src, N*sizeof(T), FGPU_COPY_CPU_TO_GPU);
    if (ret < 0)
        exit(EXIT_FAILURE);
...
{
    int ret = fgpu_memory_copy_async((void*)dst, (void*)src, N*sizeof(T), FGPU_COPY_GPU_TO_CPU);
    if (ret < 0)
        exit(EXIT_FAILURE);
...
template <typename T>
void set(T* dst, char val, int N)
{
    int ret = fgpu_memory_memset_async((void *)dst, val, N);
    if (ret < 0)
        exit(EXIT_FAILURE);
...
void dump(float* variables, int nel, int nelr)
{
	...
	{
		std::ofstream file(FILEPATH "density");
	...
	{
		std::ofstream file(FILEPATH "momentum");
		file << nel << " " << nelr << std::endl;
		for(int i = 0; i < nel; i++)
		{
	...
	{
		std::ofstream file(FILEPATH "density_energy");
		file << nel << " " << nelr << std::endl;
...
__global__
FGPU_DEFINE_KERNEL(cuda_initialize_variables, int nelr, float* variables)
{
    dim3 _blockIdx;
    FGPU_DEVICE_INIT();
...
void initialize_variables(int nelr, float* variables)
{
	...
	int ret = FGPU_LAUNCH_KERNEL(cuda_initialize_variables, Dg, Db, 0, nelr, variables);
    if (ret < 0)
        exit(EXIT_FAILURE);
}
...
__global__ 
FGPU_DEFINE_KERNEL(cuda_compute_step_factor, int nelr, float* variables, float* areas, float* step_factors)
{
    dim3 _blockIdx;
    FGPU_DEVICE_INIT();
...
void compute_step_factor(int nelr, float* variables, float* areas, float* step_factors)
{
	...
	int ret = ret = FGPU_LAUNCH_KERNEL(cuda_compute_step_factor, Dg, Db, 0, nelr, variables, areas, step_factors);
    if (ret < 0)
        exit(EXIT_FAILURE);
}
...
*/
__global__ 
FGPU_DEFINE_KERNEL(cuda_compute_flux, int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes)
{
    dim3 _blockIdx;
    FGPU_DEVICE_INIT();
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes)
{
	...
	int ret = FGPU_LAUNCH_KERNEL(cuda_compute_flux, Dg,Db, 0, nelr, elements_surrounding_elements, normals, variables, fluxes);
	if (ret < 0)
        exit(EXIT_FAILURE);
}

__global__
FGPU_DEFINE_KERNEL(cuda_time_step, int j, int nelr, float* old_variables, float* variables, float* step_factors, float* fluxes)
{
    dim3 _blockIdx;
    FGPU_DEVICE_INIT();
...
void time_step(int j, int nelr, float* old_variables, float* variables, float* step_factors, float* fluxes)
{
	...
	int ret = FGPU_LAUNCH_KERNEL(cuda_time_step, Dg,Db, 0, j, nelr, old_variables, variables, step_factors, fluxes);
    if (ret < 0)
        exit(EXIT_FAILURE);
}
...
int main(int argc, char** argv)
{
	...
	float* normals;
	{
		std::ifstream file(FILEPATH INPUT_FILENAME);
        if(!file.is_open())
        {
            exit(EXIT_FAILURE);
        }

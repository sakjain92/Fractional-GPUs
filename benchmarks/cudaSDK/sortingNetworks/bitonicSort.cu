...
)
{
    ...
    if (arrayLength <= SHARED_SIZE_LIMIT)
    {
        ...
        FGPU_LAUNCH_KERNEL(bitonicSortShared, blockCount, threadCount, 0, d_DstKey, d_DstVal, d_SrcKey, d_SrcVal, arrayLength, dir);
    }
    else
    {
	    FGPU_LAUNCH_KERNEL(bitonicSortShared1, blockCount, threadCount, 0, d_DstKey, d_DstVal, d_SrcKey, d_SrcVal);

        for (uint size = 2 * SHARED_SIZE_LIMIT; size <= arrayLength; size <<= 1)
            for (unsigned stride = size / 2; stride > 0; stride >>= 1)
                if (stride >= SHARED_SIZE_LIMIT)
                {
                    FGPU_LAUNCH_KERNEL(bitonicMergeGlobal, (batchSize * arrayLength) / 512, 256, 0, d_DstKey, d_DstVal, d_DstKey, d_DstVal, arrayLength, size, stride, dir);
                }
                else
                {
		            FGPU_LAUNCH_KERNEL(bitonicMergeShared, blockCount, threadCount, 0, d_DstKey, d_DstVal, d_DstKey, d_DstVal, arrayLength, size, dir);
                    break;
                }
    }
